...
// 修改 getDiPanStem：使用输出参数
void getDiPanStem(int nGongeIndex, char *out, size_t outSize) {
    strcpy(out, &pGlobalTianGan->pHan->szData[STRINDEX(nReDiPan[nGongeIndex])]);
}

// 修改 getTianPanStem：使用输出参数
void getTianPanStem(int nGongeIndex, char *out, size_t outSize) {
    char szTem[STR_LEN_08] = {0};
    char szTemTwo[STR_LEN_08] = {0};
    ...
    if ((strcmp(szTem, nReTianGanHan) == 0) || (strcmp(szTemTwo, nReTianGanHan) == 0)) {
        snprintf(out, outSize, "%s%s", szTem, szTemTwo);
    } else {
        snprintf(out, outSize, "%s%s", szTem, szTemTwo);
    }
}
...
// 修改 getBamen：使用输出参数
void getBamen(int nGongeIndex, char *out, size_t outSize) {
    strcpy(out, &pGlobalBamenR->pHan->szData[STRINDEX(nReBamen[nGongeIndex])]);
}

// 修改 getJiuxing：使用输出参数
void getJiuxing(int nGongeIndex, char *out, size_t outSize) {
    strcpy(out, &pGlobalJiuxingR->pHan->szData[STRINDEX(nReJiuXing[nGongeIndex])]);
}

// 修改 getBashen：使用输出参数
void getBashen(int nGongeIndex, char *out, size_t outSize) {
    strcpy(out, &pGlobalBashenR->pHan->szData[STRINDEX(nReBashen[nGongeIndex])]);
}

// 修改 getOthers：使用结构体代替数组返回
OtherFlags getOthers(int nGongeIndex) {
    OtherFlags flags = {false, false};

    if (nReOther[nGongeIndex] == 0) {
        // 两个都是 false
    } else if (nReOther[nGongeIndex] > 10) {
        flags.flag1 = true;
        flags.flag2 = true;
    } else {
        if (nReOther[nGongeIndex] == 1)
...
// 修改 PrintRowResult：避免局部指针问题，使用固定缓冲区
void PrintRowResult() {
    printf("===========================================\n");
    printf("=Row Result===================================\n");
    printf("===========================================\n");
    printf("定义甲甲=空，因为甲甲只出现在中五宫，而中五宫没有任何天盘天干，\n定义甲+{天干}的天盘干为去除甲后的天干。");

    printf("[\n");

    for (int i = 0; i < 9; ++i) {
        const char* name = gongNames[i];
        int code = i;

        char upStem[STR_LEN_08] = {0};
        char downStem[STR_LEN_08] = {0};
        char door[STR_LEN_08] = {0};
        char star[STR_LEN_08] = {0};
        char shen[STR_LEN_08] = {0};

        getTianPanStem(code, upStem, sizeof(upStem));
        getDiPanStem(code, downStem, sizeof(downStem));
        getBamen(code, door, sizeof(door));
        getJiuxing(code, star, sizeof(star));
        getBashen(code, shen, sizeof(shen));

        OtherFlags flags = getOthers(code);
        bool death = flags.flag2;
        bool active = flags.flag1;
...
void PrintResult()
{
    comlineInit();
    LineLn();
    printf("感谢奇门遁甲排盘程序CQM的开发者：taynpg (Gitee名称)\n本排盘程序使用了此程序作为内核。");
